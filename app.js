class Board {
    constructor() {
        ...
        this.boardScore = document.getElementById('score');
    }
    ...
    renderSnake() {
        ...
        if (snakeBodyElems) {
            snakeBodyElems.forEach(function(tdEl) {
                tdEl.classList.add('snakeBody');//classname set
            })
        }
    }

    renderScore(){
        this.boardScore.innerHTML = 'счет '+this.snake.score+' очков';
    }
    /**
...
//check if head headed to the body
    isNextStepToBody(nextCellCoords) {
        let result=0;
        this.snake.body.forEach(function (item,index) {
                if (index > 0){
                    if ((item.x==nextCellCoords.x) && (item.y==nextCellCoords.y)){
                        result =1;
                    }
                }
...
class Food {
     ...
     * положением на игровом поле + check its not on snake body
     * @returns {Food}
     */
...
class Game {
    ...
    isGameWon() {
        if (this.snake.body.length == this.settings.winLength) {
            clearInterval(this.tickIdentifier);
            this.setMessage('Вы выиграли, счет '+this.snake.score+' очков');
            return true;
        }
        return false;
    }
     ...
     */
    isGameLost() {
        if ((this.board.isNextStepToWall(this.snake.body[0])) || (this.board.isNextStepToBody(this.snake.newHeadCoords))){
            clearInterval(this.tickIdentifier);
...
window.addEventListener('load', () => {
    ...
    settings.init({ speed: 5, winLength: 230 });
...
class Settings {
     ...
     */
    init(params) {
        let defaultParams = {rowsCount: 21, colsCount: 21, speed: 2, winLength: 230};
       ...
       if (defaultParams.rowsCount < 10 || defaultParams.rowsCount > 30) {
        ...
        if (defaultParams.winLength < 5 || defaultParams.winLength > 2350) {
...
class Snake {
    ...
    performStep() {
        ...
        switch (this.direction) {
            case "down":
                if (this.noborder){
                    //check y is bound lower make new y-1
                    if (currentHeadCoords.y==this.settings.rowsCount){
                        //currentHeadCoords.y=-1;
                        newHeadCoords.y=0;
                    }
                }
                newHeadCoords.y++;
            ...
            case "up":
                if (this.noborder){
                    if (currentHeadCoords.y==1){
                        newHeadCoords.y=this.settings.rowsCount+1;
                    }
                }
                newHeadCoords.y--;
                break;
            case "left":
                if (this.noborder){
                    if (currentHeadCoords.x==1){
                        newHeadCoords.x=this.settings.colsCount+1;
                    }
                }
                newHeadCoords.x--;
                break;
            case "right":
                if (this.noborder){
                    //check y is bound lower make new y-1
                    if (currentHeadCoords.x==this.settings.colsCount){
                        //currentHeadCoords.y=-1;
                        newHeadCoords.x=0;
                    }
                }
        ...
        this.body.pop();
        this.newHeadCoords.x=newHeadCoords.x;
        this.newHeadCoords.y=newHeadCoords.y;
    }
